'use client'
// lib/exportReport.ts — PDF & JPG export (browser only)
// Semua import library berat dilakukan secara dynamic agar tidak error di server
...
export async function generatePDF(data: ExportData): Promise<void> {
  // Dynamic import — hanya load saat dibutuhkan di browser
  ...
  doc.text(
    `Tanggal: ${tanggal}   |   Total WH: ${warehouses.length}   |   Dibuat: ${new Date().toLocaleString('id-ID')}`,
    pageW / 2, 17, { align: 'center' }
  )

  // Table data
  const head = [['JENIS 2', 'STATUS', 'TYPE', ...shortWHs, 'Grand Total']]
  let prevJenis = ''
  ...
  const remaining = pageW - 28 - fixedW - 18
  ...
  autoTable(doc, {
    ...
    didParseCell(d) {
      ...
      const r = d.row.index, c = d.column.index
      if (c === 1) {
  ...
  doc.text(
    `NTE Operations · Telkom Indonesia · ${operator} ${area} · ${tanggal}`,
    ...
    <div style="border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,.1)">
      <div style="background:${opHex};padding:14px 20px;color:white">
        ...
        <div style="font-size:10px;opacity:.8;margin-top:4px">
          Tanggal: ${tanggal} · Total: ${totalUnit.toLocaleString('id-ID')} unit
        </div>
      </div>
      <table style="width:100%;border-collapse:collapse;font-size:11px">
        <thead>
          <tr style="background:${opHex}">
            <th style="color:white;padding:6px 8px;text-align:left;border:1px solid rgba(255,255,255,.2)">JENIS 2</th>
            <th style="color:white;padding:6px 4px;text-align:center;border:1px solid rgba(255,255,255,.2)">STATUS</th>
            <th style="color:white;padding:6px 8px;text-align:left;border:1px solid rgba(255,255,255,.2)">TYPE</th>
            ${shortWHs.map(wh =>
              `<th style="color:white;padding:6px 4px;text-align:center;font-size:9px;border:1px solid rgba(255,255,255,.2)">${wh}</th>`
            ).join('')}
            <th style="color:white;padding:6px 4px;text-align:center;background:#C0392B;border:1px solid rgba(255,255,255,.2)">TOTAL</th>
          </tr>
        </thead>
        <tbody>
          ${rows.map((r, i) => {
            ...
            return `
              <tr style="background:${bg}">
                <td style="padding:4px 8px;border:1px solid #e2eaf2;font-weight:${jenis ? 'bold' : 'normal'};
                  color:#1e3a5f;background:${jenis ? '#ebf2fa' : bg}">${jenis}</td>
                <td style="padding:4px;border:1px solid #e2eaf2;text-align:center">
                  <span style="background:${statusBg};color:${statusTxt};padding:2px 6px;
                    border-radius:4px;font-size:9px;font-weight:600">
                    ${r.status_nte === 'NTE BARU' ? 'BARU' : 'RFBSH'}
                  </span>
                </td>
                <td style="padding:4px 8px;border:1px solid #e2eaf2;font-family:monospace;font-size:10px">
                  ${r.type_nte.replace(/_/g, ' ')}
                </td>
                ${warehouses.map(wh => {
                  const v = (r[wh] || 0) as number
                  return `<td style="padding:4px;border:1px solid #e2eaf2;text-align:center;
                    background:${v > 0 ? '#f0fdf4' : bg};color:${v > 0 ? '#166534' : '#ccc'}">
                    ${v > 0 ? v : ''}
                  </td>`
                }).join('')}
                <td style="padding:4px;border:1px solid #e2eaf2;text-align:center;
                  background:#fadbd8;color:#c0392b;font-weight:bold">
                  ${r.grand_total || ''}
                </td>
              </tr>`
          }).join('')}
          <tr style="background:#1e3a5f">
            <td colspan="3" style="padding:6px 8px;color:white;font-weight:bold;border:1px solid #0d2137">
              Grand Total
            </td>
            ${warehouses.map(wh => {
              const t = rows.reduce((s, r) => s + ((r[wh] || 0) as number), 0)
              return `<td style="padding:6px 4px;text-align:center;color:white;font-weight:bold;
                background:#2e6da4;border:1px solid #0d2137">${t || ''}</td>`
            }).join('')}
            <td style="padding:6px 4px;text-align:center;color:white;font-weight:bold;
              background:#c0392b;border:1px solid #922b21">${totalUnit}</td>
          </tr>
        </tbody>
      </table>
    </div>
...
export async function generateAllReports(
  ...
  for (const [ak, cfg] of Object.entries(areaConfig)) {
    ...
    if (fmt === 'jpg') await generateJPG(d)
    else               await generatePDF(d)
